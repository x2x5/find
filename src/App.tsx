...
function AppContent() {
  ...
  const [pageSize, setPageSize] = useState(() => {
    try {
      ...
      if (stored && [10, 50, 100].includes(parseInt(stored, 10)))
  ...
  const handlePageSizeChange = useCallback((size: number) => {
    setPageSize(size);
    try {
      localStorage.setItem("page_size", String(size));
    } catch {}
  }, []);
  ...
  const [pinnedPaper, setPinnedPaper] = useState<{
    key: string;
    position: number;
  } | null>(null);
  ...
  const {
    ...
    error: papersError,
  } = usePapers(selectedConfs, yearRange, manifest);
  ...
  const shuffledPapers = useMemo(() => {
    ...
    if (
      key === prevShuffleKeyRef.current &&
      prevShuffledRef.current.length > 0
    ) {
  ...
  const handleWordClick = useCallback(
    (word: string, _paper: Paper, globalIdx: number) => {
      setSearchValue((prev) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 transition-colors duration-200">
      ...
      <main className="max-w-[1560px] mx-auto px-4 pt-2 pb-0 hidden lg:grid lg:grid-cols-[260px_minmax(0,1fr)_216px] lg:grid-rows-1 gap-4 lg:items-start [&>*]:self-start">
        <RightSidebar showTimeline={showTimeline} />

        <section className="space-y-3 min-w-0 min-h-[calc(100vh-5rem)] self-start">
          {combinedLoading && (
          ...
          {mobileTab === "settings" && (
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full flex items-center gap-3 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 py-3 text-sm"
              >
                <span className="w-5 h-5 flex items-center justify-center text-sm font-bold text-emerald-500">
                  A
                </span>
                <span className="flex-1 text-left text-zinc-700 dark:text-zinc-200">
                  {t.language.label}
