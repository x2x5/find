...
function highlightText(
  ...
  return tokens.map((token, i) => {
    ...
    const isHighlighted = escaped.some((pat) =>
      new RegExp(pat, "i").test(token),
    );
...
export default function PapersTable({
  ...
  const searchGitHub = useCallback(
    async (title: string, globalIdx: number) => {
      setGithubLoadingIdx(globalIdx);
      try {
        const query = title.split(/\s+/).slice(0, 8).join(" ");
  ...
  const pagePapers = useMemo(() => {
    ...
    return papers
      ...
      .sort((a, b) => {
        const countDiff =
          getTitleWordCount(a.paper.title) - getTitleWordCount(b.paper.title);
        if (countDiff !== 0) return countDiff;
        return a.index - b.index;
      })
      .map(({ paper }) => paper);
  }, [papers, startIdx, pageSize, infiniteScroll, displayCount]);
  const placeholderCount = infiniteScroll
    ? 0
    : Math.max(0, pageSize - pagePapers.length);
  ...
  return (
    <div
      data-papers-table
      className="w-full min-w-0 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden"
    >
      <div
        className={`border-b border-zinc-200 dark:border-zinc-800 bg-gradient-to-r from-zinc-50 via-white to-amber-50/70 dark:from-zinc-900 dark:via-zinc-900 dark:to-amber-950/20 ${infiniteScroll ? "hidden sm:block" : ""}`}
      >
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="inline-flex items-center rounded-full border border-amber-200/80 dark:border-amber-900/70 bg-white/90 dark:bg-zinc-950/80 px-3 py-1 shadow-sm">
              <div className="text-sm font-semibold text-zinc-700 dark:text-zinc-200 tabular-nums">
                {papers.length > 0
                  ? infiniteScroll
                    ? `${pagePapers.length}/${papers.length}`
                    : `${startIdx + 1}-${Math.min(startIdx + pageSize, papers.length)}`
                  : `0`}
              </div>
            </div>
            ...
            >
              ...
              <div className="flex gap-2 sm:hidden">
                <span
                  ...
                  style={opacityStyle}
                >
                  {highlightText(paper.title, searchTrigger, (word) =>
                    onWordClick?.(word, paper, globalIdx),
                  )}
                </span>
                <div className="shrink-0 flex flex-col items-end gap-1">
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className="shrink-0 w-5 h-5 flex items-center justify-center rounded-md bg-indigo-100 text-indigo-600 hover:bg-indigo-200 hover:text-indigo-700 dark:bg-indigo-950 dark:text-indigo-400 dark:hover:bg-indigo-900 dark:hover:text-indigo-300 active:scale-90 transition-all"
                      title="Copy title"
                    >
                      {copiedIdx === globalIdx ? (
                    ...
                    <button
                      ...
                      className="shrink-0 w-5 h-5 flex items-center justify-center rounded-md bg-neutral-800 text-white hover:bg-neutral-700 dark:bg-white dark:text-neutral-900 dark:hover:bg-zinc-200 active:scale-90 transition-all disabled:opacity-30"
                      title="Search GitHub"
                    >
              ...
              <div className="hidden sm:flex items-center gap-3">
                ...
                <button
                  onClick={() => searchGitHub(paper.title, globalIdx)}
                  disabled={githubLoadingIdx === globalIdx}
                  className="shrink-0 w-7 h-7 flex items-center justify-center rounded-md bg-neutral-800 text-white hover:bg-neutral-700 dark:bg-white dark:text-neutral-900 dark:hover:bg-zinc-200 active:scale-90 transition-all disabled:opacity-30"
                  title="Search GitHub"
                >
