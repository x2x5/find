...
export default function VerticalTimeline() {
  ...
  const events = useMemo(() => {
    ...
    for (const item of RAW_DATA) {
      ...
      all.push({
        ...
        date: `${currentYear}-${item.deadline}`,
  ...
  const items = useMemo(() => {
    ...
    const all: TimelineItem[] = events.map((ev) => ({ kind: 'event', event: ev }));
    // Insert today marker in correct chronological position on the left (deadline) column
    const insertIdx = all.findIndex((item) => item.event!.fullDate.getTime() > todayDate.getTime());
  ...
  const textColor = (type: 'deadline' | 'result') =>
    type === 'deadline' ? 'text-rose-600 dark:text-rose-300' : 'text-emerald-600 dark:text-emerald-300';
  const badgeBg = (type: 'deadline' | 'result') =>
    type === 'deadline' ? 'bg-rose-100 dark:bg-rose-950' : 'bg-emerald-100 dark:bg-emerald-950';
  ...
  return (
    <div className="relative">
      ...
      {items.map((item, i) => {
        if (item.kind === 'today') {
          return (
            <div key={`today-${i}`} className="relative flex items-center mb-4">
              <div className="flex-1 pr-4 text-right">
                <div className="inline-flex items-center gap-2">
                  <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-300">{t.timeline.today}</span>
                  <span className="text-xs text-zinc-400">{today.getMonth() + 1}/{today.getDate()}</span>
                </div>
              </div>
              <div className="relative z-10 w-3 h-3 rounded-full bg-indigo-500 border-2 border-white dark:border-zinc-900 shrink-0" />
        ...
        const ev = item.event!;
        const isPast = ev.fullDate.getTime() < new Date(today.getFullYear(), today.getMonth(), today.getDate()).getTime();
        const isLeft = ev.type === 'deadline';

        return (
          <div key={`${ev.conference}-${ev.type}-${i}`} className="relative flex items-start mb-3">
            {/* Left side (deadlines) */}
            <div className="flex-1 pr-4">
              {isLeft && (
                <div className={`text-right ${isPast ? 'opacity-40' : ''}`}>
                  <div className="flex items-center justify-end gap-1.5 flex-wrap">
                    <span className={`text-sm font-semibold ${textColor(ev.type)}`}>{ev.conference}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${badgeBg(ev.type)} ${textColor(ev.type)}`}>{ev.label}</span>
                  </div>
            ...
            {/* Center dot */}
            <div className={`relative z-10 w-2.5 h-2.5 rounded-full ${dotColor(ev.type)} border-2 border-white dark:border-zinc-900 shrink-0 mt-1.5`} />
